// Market Terminal — live tickers (US stocks; TV-supported), scrolling top bar,
// Quote panel, Top Movers, News & Sentiment, US Market News, USD Econ Calendar.
...
// TradingView exchange map (ensure symbols display)
const TV_EXCHANGE = {
...
// --------- Ticker bar (scrolling; pause on hover; live updates) ----------
const nodes = new Map();
...
function buildTickerRow(items){
  ...
  nodes.clear();
  const twice = [...items, ...items]; // for seamless loop
  twice.forEach(tk=>{
    ...
    // Only map first set so updates aren't doubled
    if (!nodes.has(tk.symbol)) nodes.set(tk.symbol,{item,price,chg,last:tk.price});
  });
  // trigger CSS marquee width calc
...
async function loadCalendar(){
  ...
  try{
    ...
    if (!Array.isArray(data) || !data.length){
      econCalBody.innerHTML='<tr><td colspan="6" class="muted">No data available.</td></tr>'; return;
    }
