...
(function(){
  ...
  themeToggle.addEventListener('change', ()=>{
    ...
    drawSeasonals(lastSeasonals); drawGauge(lastCompound);
  });
})();
...
(function(){
  ...
  gridRoot.addEventListener('dragstart', e=>{
    ...
    dragEl=header.parentElement; dragEl.classList.add('dragging');
    placeholder=document.createElement('section'); placeholder.className='placeholder card';
    placeholder.style.height=`${dragEl.getBoundingClientRect().height}px`; dragEl.after(placeholder);
  });
  gridRoot.addEventListener('dragover', e=>{
    if(!dragEl) return; e.preventDefault();
    const after=getAfter(gridRoot, e.clientY);
    if(after==null) gridRoot.appendChild(placeholder); else gridRoot.insertBefore(placeholder, after);
  });
...
(function initSnapResize(){
  ...
  cards.forEach(card=>{
    ...
    const onMove=(e)=>{
      ...
      let best=allowed[0], dmin=1e9;
      allowed.forEach(o=>{ const d=Math.abs(o.c-estCols)*3+Math.abs(o.r-estRows); if(d<dmin){dmin=d; best=o;}});
      const targetW = best.c===2 ? (cW*2 + GAP) : cW;
      const targetH = best.r * ROW_UNIT;
      outline.style.left=`${startRect.left}px`; outline.style.top =`${startRect.top}px`;
      outline.style.width=`${targetW}px`; outline.style.height=`${targetH}px`;
    };
    const onUp=()=>{
      ...
      const cols=(ow>cW*1.25)?2:1; const rows=Math.max(2, Math.round(oh/ROW_UNIT));
      card.classList.toggle('span-2', cols===2);
      card.dataset.cols=String(cols); card.dataset.rows=String(rows);
      card.className=card.className.replace(/\brow-\d+\b/g,'').trim(); card.classList.add(`row-${rows}`);
      document.body.removeChild(outline); outline=null; card.classList.remove('resizing');
      drawSeasonals(lastSeasonals); drawGauge(lastCompound);
    };
    handle.addEventListener('pointerdown',(e)=>{
      startRect=card.getBoundingClientRect(); startX=e.clientX; startY=e.clientY; card.classList.add('resizing');
      outline=document.createElement('div'); outline.className='resize-outline';
      outline.style.left=`${startRect.left}px`; outline.style.top =`${startRect.top}px`;
      outline.style.width=`${startRect.width}px`; outline.style.height=`${startRect.height}px`;
...
function updateTicker(items){
  items.forEach(tk=>{
    ...
    applyChangeClass(n.chg, tk.change_pct); n.chg.textContent=fmtChange(tk.change_pct);
  });
}
...
function renderPerf(perf){
  ...
  labels.forEach(k=>{
    const v=perf?perf[k]:null; const d=document.createElement('div');
    d.className='perf-box '+(v>0?'pos':(v<0?'neg':'neu'));
    d.innerHTML=`<div class="p-val">${(v==null)?'—':`${v>0?'+':''}${v.toFixed(2)}%`}</div><div class="p-lbl">${k}</div>`;
...
function renderNews(container, items){
  ...
  items.forEach(n=>{
    ...
    const meta=document.createElement('div'); meta.className='muted'; const src=n.source||'News'; const ts=n.published_at||''; meta.textContent=`${src}${ts?` · ${ts}`:''}`;
    it.append(a,meta); container.appendChild(it);
  });
}
...
async function loadMarketNews(){
  ...
  try{ renderNews(marketNewsList, await fetchJSON(ENDPOINTS.mktnews, {timeout: 12000})); }
